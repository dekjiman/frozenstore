...
const emptyForm: FormData = { title: "", subtitle: "", badgeText: "", imageUrl: "", backgroundVariant: "red", ctaLabel: "", ctaUrl: "", sortOrder: "1", isActive: true, startsAt: "", endsAt: "" };

function toFormValue(item: PromoBanner): FormData {
  return {
    title: item.title,
    subtitle: item.subtitle ?? "",
    badgeText: item.badgeText ?? "",
    imageUrl: item.imageUrl,
    backgroundVariant: item.backgroundVariant ?? "red",
...
export function AdminPromoBannersPage() {
  ...
  useEffect(() => {
    apiFetch<{ data: PromoBanner[] }>("/api/admin/promo-banners", { cache: "no-store" })
      .then((payload) => { setItems(payload.data); setError(null); })
      .catch((caught) => setError(caught instanceof Error ? caught.message : "Gagal memuat promo banner"))
  ...
  function openEdit(item: PromoBanner) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const body = {
        ...
        backgroundVariant: form.backgroundVariant,
      ...
      const saved: PromoBanner = { id: editingId ?? payload.data.id, ...body };
      setItems((prev) => editingId ? prev.map((i) => i.id === editingId ? saved : i) : [...prev, saved]);
      setNotice(`Promo "${saved.title}" berhasil ${editingId ? "diperbarui" : "ditambahkan"}.`);
      ...
      if (caught instanceof ApiError && caught.fields) {
        ...
        setFormError(caught instanceof Error ? caught.message : "Gagal menyimpan promo");
      }
    }
  }

  async function handleDelete(item: PromoBanner) {
    if (!window.confirm(`Hapus promo "${item.title}"?`)) return;
    try {
      await apiFetch(`/api/admin/promo-banners/${item.id}`, { method: "DELETE" });
      setItems((prev) => prev.filter((i) => i.id !== item.id));
      setNotice(`Promo "${item.title}" berhasil dihapus.`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Gagal menghapus promo");
    }
  }

  async function moveItem(item: PromoBanner, direction: "up" | "down") {
    ...
    try {
      await apiFetch(`/api/admin/promo-banners/${item.id}`, { method: "PATCH", body: JSON.stringify({ sortOrder: target.sortOrder }) });
      await apiFetch(`/api/admin/promo-banners/${target.id}`, { method: "PATCH", body: JSON.stringify({ sortOrder: item.sortOrder }) });
  ...
  async function toggleActive(item: PromoBanner) {
    try {
      await apiFetch(`/api/admin/promo-banners/${item.id}`, { method: "PATCH", body: JSON.stringify({ isActive: !item.isActive }) });
  ...
  const sorted = [...items].sort((a, b) => a.sortOrder - b.sortOrder);
  const variantOptions = ["red", "cream", "dark"];
    ...
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-4xl tracking-tight">Promo banner</h1>
          <p className="mt-2 text-sm text-stone-500">Kelola banner promosi di halaman beranda.</p>
        </div>
      ...
      <section className="mt-8 overflow-hidden rounded-3xl border border-stone-200 bg-white">
        <div className="border-b border-stone-200 p-5">
          <h2 className="font-serif text-2xl">Daftar promo</h2>
          <p className="mt-1 text-xs text-stone-500">{loading ? "Memuat..." : `${items.length} promo`}</p>
        </div>
            ...
            <tbody className="divide-y divide-stone-100">
              {sorted.map((item, idx) => (
                <tr key={item.id} className="hover:bg-stone-50/80">
                  ...
                  <td className="px-5 py-3 text-xs text-stone-600">{item.backgroundVariant}</td>
                  <td className="px-5 py-3">
              ...
              {!loading && items.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-stone-400">Belum ada promo banner.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
      ...
      {editingId !== undefined ? (
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-stone-950/55 p-5" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setEditingId(undefined); }}>
          <form onSubmit={handleSubmit} role="dialog" aria-modal="true" aria-labelledby="promo-form-title" className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-[var(--brand-600)]">Promo</p>
                <h2 id="promo-form-title" className="mt-1 font-serif text-3xl">{editingId ? "Edit promo" : "Tambah promo"}</h2>
              </div>
            ...
            <div className="mt-6 space-y-5">
              <FormField label="Judul" error={fieldErrors.title}>
                <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="Promo Ramadhan" className="mt-1 h-10 w-full rounded-xl border border-stone-300 bg-white px-4 text-sm" />
              </FormField>
              <FormField label="Subtitle" error={fieldErrors.subtitle}>
                <input value={form.subtitle} onChange={(e) => setForm((f) => ({ ...f, subtitle: e.target.value }))} className="mt-1 h-10 w-full rounded-xl border border-stone-300 bg-white px-4 text-sm" />
              </FormField>
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <FormField label="CTA Label" error={fieldErrors.ctaLabel}>
                  <input value={form.ctaLabel} onChange={(e) => setForm((f) => ({ ...f, ctaLabel: e.target.value }))} className="mt-1 h-10 w-full rounded-xl border border-stone-300 bg-white px-4 text-sm" />
                </FormField>
                <FormField label="CTA URL" error={fieldErrors.ctaUrl}>
                  <input value={form.ctaUrl} onChange={(e) => setForm((f) => ({ ...f, ctaUrl: e.target.value }))} className="mt-1 h-10 w-full rounded-xl border border-stone-300 bg-white px-4 text-sm" />
                </FormField>
              </div>
              <div className="grid gap-5 sm:grid-cols-2">
