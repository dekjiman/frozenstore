...
export function CheckoutFlow() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-12 lg:px-10 lg:py-16">
      ...
      <div className="mt-7">
        <p className="text-sm font-semibold text-orange-700">Selesaikan pesanan</p>
                    ...
                    className={`grid size-10 shrink-0 place-items-center rounded-full border transition ${
                      isComplete
                        ...
                        : isCurrent
                          ...
                          : "border-stone-300 bg-white text-stone-500 group-hover:border-stone-500"
                    } disabled:cursor-not-allowed`}
                  >
                    {isComplete ? <Check size={17} /> : <Icon size={17} />}
                  </span>
                  <span>
                    <span className="block text-xs text-stone-500">Langkah {step.id}</span>
                    <span className="mt-0.5 block text-sm font-semibold text-stone-900">{step.title}</span>
                    <span className="mt-0.5 block text-xs text-stone-500">{step.description}</span>
                  </span>
                </button>
                {index < steps.length - 1 ? (
                  <span
                    aria-hidden="true"
                    className={`mx-4 mt-5 h-px min-w-8 flex-1 ${step.id < currentStep ? "bg-emerald-500" : "bg-stone-300"}`}
                  />
                ) : null}
              </li>
            );
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start lg:gap-12">
        <section className="rounded-3xl border border-stone-200 bg-white p-6 sm:p-8" aria-live="polite">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-orange-700">
            Langkah {currentStep} dari {steps.length}
          </p>
          <h2 className="mt-2 font-serif text-3xl text-stone-900">{steps[currentStep - 1].title}</h2>
          <p className="mt-3 max-w-xl text-sm leading-6 text-stone-500">
            {currentStep === 1
              ...
              : currentStep === 2
                ? "Periksa total pesanan dan ikuti panduan transfer ke rekening resmi Raf Store."
                : currentStep === 3
          ...
          {currentStep < 4 ? <div className="mt-7 flex items-center justify-between gap-4">
            <button
              ...
              className="rounded-full border border-stone-300 px-5 py-3 text-sm font-semibold text-stone-700 transition hover:bg-stone-100 disabled:cursor-not-allowed disabled:opacity-40"
            >
              Kembali
            </button>
            {currentStep < 3 ? (
              <button
                ...
                className="rounded-full bg-stone-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-orange-700 disabled:cursor-not-allowed disabled:bg-stone-300"
              >
                {isSubmitting ? "Menyimpan pesanan..." : "Lanjutkan"}
              </button>
            ) : (
              <p className="max-w-xs text-right text-xs leading-5 text-stone-500">
        ...
        <aside className="rounded-3xl bg-stone-950 p-6 text-white lg:sticky lg:top-8">
            ...
            {(order ? orderedItems : items).map((item) => (
              <div key={item.id} className="flex items-start justify-between gap-4 text-sm">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-stone-400">{item.quantity} × {rupiahFormatter.format(item.product.price)}</p>
                </div>
                <p className="shrink-0 text-stone-300">{rupiahFormatter.format(item.product.price * item.quantity)}</p>
              </div>
            ))}
          </div>
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between gap-4 pt-2 text-base font-semibold">
              <dt>Total</dt>
              <dd className="text-orange-300">{rupiahFormatter.format(order?.totalAmount ?? subtotal + 20_000)}</dd>
            </div>
          </dl>
