...
import { ArrowDown, ArrowUp, Edit3, Plus, Shield, Truck, Award, Headphones, Trash2, X } from "lucide-react";
...
const emptyForm: FormData = { iconKey: "shield", title: "", description: "", sortOrder: "1", isActive: true };
const iconOptions = [
...
function toFormValue(item: TrustItem): FormData {
  return {
...
export function AdminTrustItemsPage() {
  ...
  useEffect(() => {
    apiFetch<{ data: TrustItem[] }>("/api/admin/trust-items", { cache: "no-store" })
      .then((payload) => { setItems(payload.data); setError(null); })
      .catch((caught) => setError(caught instanceof Error ? caught.message : "Gagal memuat trust items"))
  ...
  function openEdit(item: TrustItem) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.iconKey.trim()) { setFormError("Ikon wajib dipilih."); return; }
    ...
    try {
      ...
      const payload = await apiFetch<{ data: { id: string } }>(
        editingId ? `/api/admin/trust-items/${editingId}` : "/api/admin/trust-items",
        { method: editingId ? "PATCH" : "POST", body: JSON.stringify(body) },
      );
      const saved: TrustItem = { id: editingId ?? payload.data.id, ...body };
      setItems((prev) => editingId ? prev.map((i) => i.id === editingId ? saved : i) : [...prev, saved]);
      setNotice(`Trust item "${saved.title}" berhasil ${editingId ? "diperbarui" : "ditambahkan"}.`);
      ...
      if (caught instanceof ApiError && caught.fields) {
        ...
        setFormError(caught instanceof Error ? caught.message : "Gagal menyimpan trust item");
      }
    }
  }

  async function handleDelete(item: TrustItem) {
    if (!window.confirm(`Hapus trust item "${item.title}"?`)) return;
    try {
      await apiFetch(`/api/admin/trust-items/${item.id}`, { method: "DELETE" });
      setItems((prev) => prev.filter((i) => i.id !== item.id));
      setNotice(`Trust item "${item.title}" berhasil dihapus.`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Gagal menghapus trust item");
    }
  }

  async function moveItem(item: TrustItem, direction: "up" | "down") {
    ...
    try {
      await apiFetch(`/api/admin/trust-items/${item.id}`, { method: "PATCH", body: JSON.stringify({ sortOrder: target.sortOrder }) });
      await apiFetch(`/api/admin/trust-items/${target.id}`, { method: "PATCH", body: JSON.stringify({ sortOrder: item.sortOrder }) });
  ...
  async function toggleActive(item: TrustItem) {
    try {
      await apiFetch(`/api/admin/trust-items/${item.id}`, { method: "PATCH", body: JSON.stringify({ isActive: !item.isActive }) });
    ...
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-4xl tracking-tight">Trust items</h1>
          <p className="mt-2 text-sm text-stone-500">Kelola elemen kepercayaan di halaman beranda.</p>
        </div>
        <button type="button" onClick={openNew} className="inline-flex h-11 items-center gap-2 rounded-full bg-stone-900 px-5 text-sm font-semibold text-white hover:bg-[var(--brand-700)]">
          <Plus size={17} />Tambah trust item
        </button>
      </div>
      ...
      <section className="mt-8 overflow-hidden rounded-3xl border border-stone-200 bg-white">
        <div className="border-b border-stone-200 p-5">
          <h2 className="font-serif text-2xl">Daftar trust item</h2>
          <p className="mt-1 text-xs text-stone-500">{loading ? "Memuat..." : `${items.length} item`}</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[550px] text-left text-sm">
            ...
            <tbody className="divide-y divide-stone-100">
              {sorted.map((item, idx) => {
                const iconDef = iconOptions.find((o) => o.value === item.iconKey);
                const IconComp = iconDef?.Icon ?? Shield;
                return (
                  <tr key={item.id} className="hover:bg-stone-50/80">
                    ...
                    <td className="px-5 py-3"><span className="grid size-9 place-items-center rounded-xl bg-stone-100 text-stone-700"><IconComp size={18} /></span></td>
              ...
              {!loading && items.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-stone-400">Belum ada trust item.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
      ...
      {editingId !== undefined ? (
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-stone-950/55 p-5" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setEditingId(undefined); }}>
          <form onSubmit={handleSubmit} role="dialog" aria-modal="true" aria-labelledby="trust-form-title" className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-[var(--brand-600)]">Trust Item</p>
                <h2 id="trust-form-title" className="mt-1 font-serif text-3xl">{editingId ? "Edit trust item" : "Tambah trust item"}</h2>
              </div>
            ...
            <div className="mt-6 space-y-5">
              <FormField label="Ikon" error={fieldErrors.iconKey}>
                <div className="mt-2 flex flex-wrap gap-3">
                  {iconOptions.map((opt) => {
                    ...
                    return (
                      <button key={opt.value} type="button" onClick={() => setForm((f) => ({ ...f, iconKey: opt.value }))} className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold transition-colors ${form.iconKey === opt.value ? "border-stone-900 bg-stone-900 text-white" : "border-stone-300 bg-white text-stone-700 hover:bg-stone-50"}`}>
              ...
              <FormField label="Judul" error={fieldErrors.title}>
                <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="Pengiriman cepat" className="mt-1 h-10 w-full rounded-xl border border-stone-300 bg-white px-4 text-sm" />
              </FormField>
