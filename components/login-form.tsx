...
export function LoginForm({ forAdmin = false }: { forAdmin?: boolean }) {
  ...
  return (
    <form onSubmit={submit} noValidate className="rounded-3xl border border-[var(--border)] bg-white p-6 shadow-[0_24px_70px_-45px_rgba(28,25,23,0.45)] sm:p-8">
      ...
      <h1 className="mt-5 font-serif text-3xl text-[var(--ink-950)]">{forAdmin ? "Masuk ke panel admin" : "Masuk ke Jasmine Shop Premium Product"}</h1>
      <p className="mt-2 text-sm leading-6 text-[var(--ink-700)]">{forAdmin ? "Gunakan email dan password akun admin." : "Akses riwayat pesanan dan lanjutkan checkout dengan akun pelangganmu."}</p>
