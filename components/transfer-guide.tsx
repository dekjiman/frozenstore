...
export function TransferGuide({ total }: { total: number }) {
  ...
  return <div className="mt-8 space-y-6">
    <div className="rounded-2xl bg-orange-50 p-5 sm:flex sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-orange-700">Total pembayaran</p><p className="mt-2 font-serif text-3xl font-semibold">{rupiahFormatter.format(total)}</p></div><button type="button" onClick={() => void copyValue(String(total))} className="mt-4 inline-flex items-center gap-2 rounded-full border border-orange-200 bg-white px-4 py-2 text-xs font-semibold text-orange-800 sm:mt-0">{copiedValue === String(total) ? <Check size={15} /> : <Copy size={15} />}Salin total</button></div>
    {error ? <p className="rounded-xl bg-red-50 p-4 text-sm text-red-700">{error}</p> : null}
    <fieldset><legend className="text-sm font-semibold">Pilih rekening tujuan</legend><div className="mt-3 grid gap-3 sm:grid-cols-2">{accounts.map((account) => <label key={account.id} className={`cursor-pointer rounded-2xl border p-4 ${selectedAccountId === account.id ? "border-orange-600 bg-orange-50" : "border-stone-200"}`}><input type="radio" name="bankAccount" checked={selectedAccountId === account.id} onChange={() => setSelectedAccountId(account.id)} className="sr-only" /><span className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-full bg-white text-orange-700"><Landmark size={17} /></span><span><span className="block text-sm font-semibold">{account.bankName}</span><span className="text-xs text-stone-500">Transfer bank</span></span></span></label>)}</div></fieldset>
