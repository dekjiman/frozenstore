...
const emptyForm: FormData = { stepNumber: "1", title: "", description: "", imageUrl: "", sortOrder: "1", isActive: true };

function toFormValue(item: CookingStep): FormData {
  return {
...
export function AdminCookingStepsPage() {
  const [items, setItems] = useState<CookingStep[]>([]);
  ...
  useEffect(() => {
    apiFetch<{ data: CookingStep[] }>("/api/admin/cooking-steps", { cache: "no-store" })
      .then((payload) => { setItems(payload.data); setError(null); })
      .catch((caught) => setError(caught instanceof Error ? caught.message : "Gagal memuat cooking steps"))
      .finally(() => setLoading(false));
  }, []);

  function openNew() {
    setEditingId(null);
    const maxStep = items.reduce((max, i) => Math.max(max, i.stepNumber), 0);
    setForm({ ...emptyForm, stepNumber: String(maxStep + 1), sortOrder: String(items.length + 1) });
    setFormError(null);
    setFieldErrors({});
    setNotice(null);
  }

  function openEdit(item: CookingStep) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const stepNumber = Number(form.stepNumber);
    const sortOrder = Number(form.sortOrder);
    if (!Number.isInteger(stepNumber) || stepNumber < 1) { setFormError("Nomor langkah harus minimal 1."); return; }
    ...
    try {
      ...
      const payload = await apiFetch<{ data: { id: string } }>(
        editingId ? `/api/admin/cooking-steps/${editingId}` : "/api/admin/cooking-steps",
        { method: editingId ? "PATCH" : "POST", body: JSON.stringify(body) },
      );
      const saved: CookingStep = { id: editingId ?? payload.data.id, ...body };
      setItems((prev) => editingId ? prev.map((i) => i.id === editingId ? saved : i) : [...prev, saved]);
      setNotice(`Langkah "${saved.title}" berhasil ${editingId ? "diperbarui" : "ditambahkan"}.`);
      ...
      if (caught instanceof ApiError && caught.fields) {
        ...
        setFormError(caught instanceof Error ? caught.message : "Gagal menyimpan langkah");
      }
    }
  }

  async function handleDelete(item: CookingStep) {
    if (!window.confirm(`Hapus langkah "${item.title}"?`)) return;
    try {
      await apiFetch(`/api/admin/cooking-steps/${item.id}`, { method: "DELETE" });
      setItems((prev) => prev.filter((i) => i.id !== item.id));
      setNotice(`Langkah "${item.title}" berhasil dihapus.`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Gagal menghapus langkah");
    }
  }

  async function toggleActive(item: CookingStep) {
    try {
      await apiFetch(`/api/admin/cooking-steps/${item.id}`, { method: "PATCH", body: JSON.stringify({ isActive: !item.isActive }) });
    ...
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-4xl tracking-tight">Cara memasak</h1>
          <p className="mt-2 text-sm text-stone-500">Kelola langkah-langkah memasak yang ditampilkan ke pelanggan.</p>
        </div>
      ...
      <section className="mt-8 overflow-hidden rounded-3xl border border-stone-200 bg-white">
        <div className="border-b border-stone-200 p-5">
          <h2 className="font-serif text-2xl">Daftar langkah</h2>
          <p className="mt-1 text-xs text-stone-500">{loading ? "Memuat..." : `${items.length} langkah`}</p>
        </div>
          ...
          <table className="w-full min-w-[600px] text-left text-sm">
            <thead className="bg-stone-50 text-xs uppercase tracking-wide text-stone-500">
              <tr>
                ...
                <th className="px-5 py-3 font-semibold">Langkah</th>
            ...
            <tbody className="divide-y divide-stone-100">
              {sorted.map((item) => (
                <tr key={item.id} className="hover:bg-stone-50/80">
                  <td className="px-5 py-3 text-sm font-semibold text-stone-700">{item.sortOrder}</td>
                  <td className="px-5 py-3">
                    <span className="grid size-9 place-items-center rounded-full bg-stone-900 text-xs font-bold text-white">{item.stepNumber}</span>
                  </td>
              ...
              {!loading && items.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-stone-400">Belum ada langkah memasak.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
      ...
      {editingId !== undefined ? (
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-stone-950/55 p-5" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setEditingId(undefined); }}>
          <form onSubmit={handleSubmit} role="dialog" aria-modal="true" aria-labelledby="step-form-title" className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-[var(--brand-600)]">Cara Memasak</p>
                <h2 id="step-form-title" className="mt-1 font-serif text-3xl">{editingId ? "Edit langkah" : "Tambah langkah"}</h2>
              </div>
            ...
            <div className="mt-6 space-y-5">
              <div className="grid gap-5 sm:grid-cols-2">
                <FormField label="Nomor Langkah" error={fieldErrors.stepNumber}>
                  <input type="number" min="1" value={form.stepNumber} onChange={(e) => setForm((f) => ({ ...f, stepNumber: e.target.value }))} className="mt-1 h-10 w-full rounded-xl border border-stone-300 bg-white px-4 text-sm" />
                </FormField>
              ...
              <FormField label="Judul" error={fieldErrors.title}>
                <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="Panaskan minyak" className="mt-1 h-10 w-full rounded-xl border border-stone-300 bg-white px-4 text-sm" />
              </FormField>
              ...
              <AdminImageUpload value={form.imageUrl} onChange={(url) => setForm((f) => ({ ...f, imageUrl: url }))} folder="cooking" accept="image/jpeg,image/png,image/webp,image/avif" maxSizeMB={5} error={fieldErrors.imageUrl} />
