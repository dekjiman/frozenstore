...
export function AdminStockInForm({ initialProductId = "" }: { initialProductId?: string }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-6"><p className="text-sm font-semibold text-[var(--brand-600)]">Pergerakan gudang</p><h1 className="mt-1 font-serif text-4xl tracking-tight">Tambah stok masuk</h1><p className="mt-2 text-sm text-stone-500">Catat penerimaan barang agar stok terkini dan riwayat gudang tetap sinkron.</p></div>
      ...
      <form onSubmit={submit} className="mt-7 rounded-3xl border border-stone-200 bg-white p-6 sm:p-8">
        ...
        <button type="submit" disabled={isSubmitting} className="mt-7 w-full rounded-full bg-stone-900 px-6 py-3.5 text-sm font-semibold text-white hover:bg-[var(--brand-700)] disabled:bg-stone-400">{isSubmitting ? "Mencatat..." : "Catat stok masuk"}</button>
      </form>
    </div>
  );
}
